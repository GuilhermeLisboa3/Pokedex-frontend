...
import { type ApiPokemon, type SpeciesPokemon } from '@/domain/models'
...
export const GetDataPokemonUseCase: Setup = (url, httpClient) => async ({ name }) => {
  const pokemon: HttpResponse<ApiPokemon> = await httpClient.request({ url: `${url}/pokemon/${name}`, method: 'get' })
